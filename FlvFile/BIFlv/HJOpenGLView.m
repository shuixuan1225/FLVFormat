...
- (void)setupBuffers
{
	...
	[_context renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer *)_eaglLayer];
...
//        NSLog(@"id %d", CVOpenGLESTextureGetName(_chromaTexture));
	...
	// 设置四顶点相对于视频的方向和长宽比。
	CGRect vertexSamplingRect = AVMakeRectWithAspectRatioInsideRect(CGSizeMake(_backingWidth, _backingHeight), _eaglLayer.bounds);
    
	// 计算标准化的四个坐标来绘制帧。
	CGSize normalizedSamplingSize = CGSizeMake(0.0, 0.0);
	CGSize cropScaleAmount = CGSizeMake(vertexSamplingRect.size.width/_eaglLayer.bounds.size.width, vertexSamplingRect.size.height/_eaglLayer.bounds.size.height);
