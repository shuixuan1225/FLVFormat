...
void initVideoToolBox (void);
void didDecompress(void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef pixelBuffer, CMTime presentationTimeStamp, CMTime presentationDuration );
CVPixelBufferRef decode(uint8_t* packetBuffer,uint32_t packetSize);
    ...
    uint32_t packetSize = inputPackage->packetSize;
        ...
        *pNalSize = CFSwapInt32HostToBig(nalSize);
        ...
        switch (nalType) {
            case 0x05:
                //                NSLog(@"*********** IDR frame, I帧");
                initVideoToolBox(); // 当读入IDR帧的时候初始化VideoToolbox，并开始同步解码
                pixelBuffer = decode(packetBuffer,packetSize); // 解码得到的CVPixelBufferRef会传入OpenGL ES类进行解析渲染
                break;
            case 0x07:
                //                NSLog(@"*********** SPS");
        ...
        if(pixelBuffer) {
            bi_video_output_package* package = (bi_video_output_package*)bi_malloc(sizeof(bi_video_output_package));
            package->timestamp = inputPackage->timestamp;
            package->timestamp_extends = inputPackage->timestamp_extends;
            package->cts = inputPackage->cts;
            package->bufferRef = pixelBuffer;
            package->type = inputPackage->type;
            return package;
        }
    return NULL;
}
